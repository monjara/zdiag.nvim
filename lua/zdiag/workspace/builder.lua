...
local function append_source_line(workspace, source_line, diagnostics)
  ...
  for _, diagnostic in ipairs(diagnostics) do
    ...
    table.insert(
      ...
      Decoration:new_diagnostic(workspace_row, diagnostic.col, diagnostic, #source_line)
    )
  end

  if highest_severity then
    table.insert(workspace.line_highlights, LineHighlight:new(workspace_row, highest_severity))
  end
end
...
local function append_range(workspace, bufnr, range, line_count, diagnostics_by_line)
  ...
  for index, source_line in ipairs(source_lines) do
    ...
    append_source_line(workspace, source_line, diagnostics_by_line[source_lnum] or {})
  end
...
local function append_buffer(workspace, buffer)
  ...
  table.insert(workspace.headers, Header:new(#workspace.lines, '▼ ' .. require('zdiag.fs').get_relative_path(bufnr)))
  ...
  for _, range in ipairs(ranges) do
    ...
    previous_separator_row = append_range(workspace, bufnr, range, line_count, diagnostics_by_line)
  end
end
