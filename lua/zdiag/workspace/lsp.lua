...
local function visual_range(workspace)
  ...
  if mode == 'V' then
    ...
    local line = vim.api.nvim_buf_get_lines(workspace.bufnr, end_row, end_row + 1, false)[1] or ''
  ...
  local source = require('zdiag.workspace.source')
  local start_position = source.get_position(workspace, start_row, start_col)

  local end_position = source.get_position(workspace, end_row, end_col)
...
---Request LSP code actions for the source represented by the workspace cursor.
---
---@param workspace zdiag.Workspace
---@param opts? vim.lsp.buf.code_action.Opts
function M.code_action(workspace, opts)
  opts = vim.deepcopy(opts or {})

  local mode = vim.api.nvim_get_mode().mode

  if not opts.range and (mode == 'v' or mode == 'V' or mode == '\22') then
    opts.range = visual_range(workspace)
